...
const ImageInput = ({ label, value, onChange, disabled }) => {
	return (
		<Form.Group>
			...
			{value && !disabled ? (
				...
				<img src={value} alt={label} style={{ maxWidth: "100px", marginTop: "8px" }} />
			) : null}
		</Form.Group>
	);
};
						...
						(image, index) =>
							index % 3 === 0 && (
								<Row key={index} className="mb-3">
									{formData?.images.slice(index, index + 3).map((image, colIndex) => (
										<Col key={colIndex}>
											<img
												src={
				...
				<div>
					{initialData?.images.map(
						(image, index) =>
							index % 3 === 0 && (
								<Row key={index} className="mb-3">
									{initialData?.images.slice(index, index + 3).map((image, colIndex) => (
										<Col key={colIndex}>
											<img
												src={image}
												alt={`Image ${index + colIndex + 1}`}
